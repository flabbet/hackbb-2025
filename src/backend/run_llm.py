...
WHISPER_MODEL_DIR = "./backend/models"  # Directory where whisper models are stored
...
def translate_and_summarize(
    ...
    whisper_command = f'./backend/bin/whisper-cli -m ./backend/models/ggml-{whisper_model_name}.bin -f "{audio_file_wav}" > {output_file}'
    result = subprocess.run(whisper_command, shell=True, capture_output=True)
    ...
    os.makedirs("/tmp/humi", exist_ok=True)
    transcript_file = "/tmp/humi/transcript.txt"
...
if __name__ == "__main__":
    ...
    (summary, transcript) = translate_and_summarize("/tmp/humi/humi_recording.wav", "", whisper_models[0], ollama_models[0])

    print(summary)
    summary_file = "/tmp/humi/summary.txt"
